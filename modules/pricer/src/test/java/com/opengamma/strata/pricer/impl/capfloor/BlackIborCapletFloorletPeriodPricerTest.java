...
public class BlackIborCapletFloorletPeriodPricerTest {
  ...
  private void testSurfaceSensitivity(
    ...
    for (int i = 0; i < nParams; ++i) {
      ...
      assertEquals(computed.getSensitivity().get(i),
          expectedMap.get(DoublesPair.of(metaParam.getYearFraction(), metaParam.getStrike().getValue())), NOTIONAL * EPS_FD);
    }
  }
  ...
  private static final IborCapletFloorletPeriod CAPLET_REG = IborCapletFloorletPeriod.builder()
      .caplet(0.04)
      .notional(NOTIONAL)
      .rateObservation(IborRateObservation.of(EUR_EURIBOR_3M, FIXING))
      .build();
